...
import { SunIcon, MoonIcon, ChevronDownIcon, CloseIcon } from "@/components/ui/Icon"; 
...
function MobileNavItem({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <li>
      <PopoverButton as={NavLink} to={to} className="block py-2">
        {children}
      </PopoverButton>
    </li>
  );
...
function ThemeToggle() {
  ...
  return (
    ...
    >
      <SunIcon className="h-6 w-6 fill-zinc-100 stroke-zinc-500 transition group-hover:fill-zinc-2 00 group-hover:stroke-zinc-700 dark:hidden [@media(prefers-color-scheme:dark)]:fill-teal-50 [@media(prefers-color-scheme:dark)]:stroke-teal-500 [@media(prefers-color-scheme:dark)]:group-hover:fill-teal-50 [@media(prefers-color-scheme:dark)]:group-hover:stroke-teal-600" />
...
function Avatar({
  ...
  className,
  ...props
}: Omit<React.ComponentPropsWithoutRef<typeof Link>, "to"> & { large?: boolean }) {
  return (
  ...
  return (
    <>
      ...
      >
        ...
        <div ref={headerRef} className="top-0 z-10 h-16 pt-6" style={{ position: "var(--header-position)" as React.CSSProperties["position"] }}>
          <Container
            className="top-(--header-top,--spacing(6)) w-full"
            style={{ position: "var(--header-inner-position)" as React.CSSProperties["position"] }}
          >
            <div className="relative flex gap-4">
              <div className="flex flex-1">{!isHomePage && <AvatarContainer><Avatar /></AvatarContainer>}</div>
